...
class FrameModule_CV(Module):
    ...
    def onStart(self, parent, path):
        ...
        self._log.debug('Press '+term.BOLD+term.CYAN+'ESC'+term.END +
                        " in '{}' to finish.".format(self._frame_name))
        return True
...
class Al5dModule(Module):
    ...
    def onStart(self, parent, path):
        ...
        if err is not None:
            self._log.warn('AL5D arm failed to initialize - {}'.format(err))
            return False
...
class SingleSyncModule(Module):
    ...
    def onStart(self, parent, path):
        ...
        self._time_sync = ext.TimeSync(self._mod.getTimes(), start_time,
                                       sync_ms)
...
class DoubleSyncModule(Module):
    ...
    def onStart(self, parent, path):
        ...
        diff_ms = ext.delta_ms(
            self._mod_a.getDatetime() - self._mod_b.getDatetime())
        ...
        self._time_sync_a = ext.TimeSync(self._mod_a.getTimes(), start_time,
                                         a_ms)
        self._time_sync_b = ext.TimeSync(self._mod_b.getTimes(), start_time,
                                         b_ms)
...
class ModuleCamera(VideoCaptureModule):
    ...
    def onStart(self, parent, path):
        ...
        if not self._cap.isOpened():
            self._log.warn(
                        'Video capture failed - no video capture device found')
            return False
        return True
...
class ModuleCamera_Video(Module):
    ...
    def onStart(self, parent, path):
        ...
        self._out = cv2.VideoWriter(self._path+self._file, fourcc, fps,
...
class ModulePs4(Module):
    ...
    def onStart(self, parent, path):
        ...
        self._log.debug('Press ' + term.BOLD + term.GREEN+'TRIANGLE' +
                        term.END + ' on the PS4 controller to save and exit.')
        self._log.debug('Press ' + term.BOLD + term.RED + 'CIRCLE' + term.END +
                        ' on the PS4 controller to discard and exit.')
        return True
...
class ModuleAl5d_Ps4(Module):
    ...
    def onUpdate(self, parent):
        ...
        # To avoid sending too much data to arm which may cause IOError (REF#1)
