...
class VaeganTrainer(object):
    ...
    def __init__(self):
        self._project = main.ConsoleProject().project() # Contains raw data directory
        
        self._net = vaegan.VAEGAN()
        
        self._trainer = trainer.Trainer(self._net, seed=1241, epochs=10, batches=100)
        ...
        self._network_saver = saver.NetworkSaver('vaegan/models/', net=self._net)
    ...
    def train(self):
        ...
        X = [ np.zeros(3*dim*dim,dtype='float32').reshape((3,dim,dim)) for i in range(1000) ]
        batch = X[0:100]
        c = 100
    ...
    def _image_interval(self, epoch, batch, result):
        ...
        def save_image(path):
            Image.fromarray(image).save(path + '{1:03d}_{2:07d}.png'.format(epoch, batch))
    ...
    def _batch_interval(self, epoch, batch, result):
        ...
        sys.stdout.write('\r' + str(i/batch_size) + 'errors: {0:0.4f} {1:0.4f} {2:0.8f} {3:0.4f} {4:0.4f}'.format(loss_enc, loss_gen, loss_dis, loss_rnn, loss_rec))
        sys.stdout.flush()
