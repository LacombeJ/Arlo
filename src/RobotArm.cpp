...
void RobotArm::create()
{
    ...
    if(init())
    {
        ...
        //std::exit(1); //TODO jonathan remove comment?
    }
    int use_leap = 1; //TODO jonathan remove hardcoded
        ...
        if(position[i] != new_pos[i]) // detected move change
        {
            ...
            */
            if(check_ranges(ranges[i], new_pos[i]) == 0) // moved in safe ranges
            {
                ...
                //TODO jonathan Check this logic
                //...should this be equivalent to the above commented code?
            }
...
void RobotArm::set_to_mid()
{
    ...
    // set to mid slowly
    unsigned char cmd[] = "#0P1500S200#1P1600S200#2P1400S200#3P1500S200#4P1450S200#5P1400S250\r";
...
void RobotArm::commandRobotUsingIK(
    float x,
    float y,
    float z,
    float wrist_degree,
    float wrist_rotate_degree,
    float open,
    float reward)
{
    ...
    std::cout
        ...
        << " " << new_pos[SHOULDER]
        << " " << new_pos[ELBOW]
        << " " << new_pos[WRIST_ROTATE]
        << " " << new_pos[WRIST]
        << " " << new_pos[GRIPPER]
        << std::endl;

    // filtration
    if(filter != 0) //TODO jonathan what is filtration? is this beging used? do we need?
    {
        int * tmp_pos = get_pos();
        ...
        {
            // move only if greater than filter
            if(fabs((tmp_pos[i] - new_pos[i]) / (double)filter_const) > filter)
            {
                move(new_pos);
                break;
            }
